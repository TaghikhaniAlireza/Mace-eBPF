...
static void test_arena_lifecycle(void) {
    AegisArenaHandle *arena = aegis_arena_new(16);
    assert(arena != NULL && "aegis_arena_new should return non-null");

    aegis_arena_free(arena);
...
static void test_arena_push_pop(void) {
    ...
    int push_result = aegis_arena_try_push(arena, &event);
    assert(push_result == 0 && "push should succeed");

    RawMemoryEvent out;
    memset(&out, 0, sizeof(out));
    int pop_result = aegis_arena_try_pop(arena, &out);
...
static void test_arena_pop_empty(void) {
    AegisArenaHandle *arena = aegis_arena_new(4);
    assert(arena != NULL);

    RawMemoryEvent out;
    memset(&out, 0, sizeof(out));
    int result = aegis_arena_try_pop(arena, &out);
...
static void test_alert_channel_lifecycle(void) {
    AegisAlertChannelHandle *ch = aegis_alert_channel_new(32);
    assert(ch != NULL && "aegis_alert_channel_new should return non-null");

    aegis_alert_channel_free(ch);
...
static void test_alert_channel_recv_empty(void) {
    ...
    uint8_t buffer[4096];
    int result = aegis_alert_channel_try_recv(ch, buffer, sizeof(buffer));
...
static void test_null_safety(void) {
    ...
    aegis_arena_free(NULL);
    aegis_alert_channel_free(NULL);

    RawMemoryEvent event;
    memset(&event, 0, sizeof(event));
    int r1 = aegis_arena_try_push(NULL, &event);
    assert(r1 < 0 && "null handle should return error");

    uint8_t buf[64];
    int r2 = aegis_alert_channel_try_recv(NULL, buf, sizeof(buf));
...
/* ── Main ────────────────────────────────────────────────── */
int main(void) {
    printf("=== Aegis-eBPF C Smoke Test ===\n\n");
