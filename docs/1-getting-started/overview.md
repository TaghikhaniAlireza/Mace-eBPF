...
**Aegis-eBPF** is a security-focused **Linux eBPF** monitoring stack. It attaches **syscall tracepoint** programs (for example `mmap`, `mprotect`, `execve`, `openat`, `ptrace`, `memfd_create`), streams observations to userspace through a **ring buffer**, and evaluates **YAML** rules against enriched events.
...
| **Userspace core** | Rust (`aegis-ebpf` crate) | Load CO-RE BPF object, reorder/enrich events, run rule engine and suppressions, optional k8s enrichment. |
| **FFI** | C ABI (`aegis.h`, `libaegis_ebpf.so` / static `.a`) | Embedded engines, Go `cgo`, Python `ctypes`. |
| **Agent** | Go (`aegis-agent`) | CLI, file-backed config, structured event log, systemd integration. |
| **Distribution** | `.deb` (nFPM), OCI (GHCR), tarball | Pre-built binaries and libraries for integrators. |

## Design principles (as implemented)

- **CO-RE–friendly layouts** in `aegis-ebpf-common` so a single built BPF object can target multiple kernels when BTF and verifier constraints allow.
...
- **Stable event export**: after evaluation, the pipeline emits a **JSON** view (`StandardizedEvent` in Rust, `AegisEvent` in Go) suitable for SIEM pipelines and agents.
