# Core logging (`AEGIS_LOG_LEVEL` and Rust diagnostics)

This document explains how **Aegis log levels** work in the Rust core, how they differ from **`RUST_LOG`**, and why you may still see **“suppressed”** text when the floor is **`ALERT`**.
...
```bash
# From repo root — Rust tests (in-memory capture of [Aegis] lines)
