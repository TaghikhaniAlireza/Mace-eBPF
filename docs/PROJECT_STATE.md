...
3. **Execve argv policy:** **Default** nested eBPF build uses **`execve_no_user_argv`** (no user argv reads in `sys_enter_execve`; rules use **`/proc/<tgid>/cmdline`** haystack). **`MACE_EBPF_EXECVE_FULL_ARGV=1`** enables v13 in-kernel capture (up to **4** args × **63** bytes per arg, **192**-byte blob). **`is_truncated`** indicates overflow. **`BPF_PROG_LOAD` EACCES** may be lockdown/capabilities. **`MACE_EBPF_EXECVE_ARGV0_ONLY=1`** is an intermediate shrink if full argv fails verifier load.
