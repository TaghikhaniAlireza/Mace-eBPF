...
3. **Execve argv policy:** **Default** nested eBPF build uses **`execve_no_user_argv`** (no user argv reads on **`sys_enter_execve`** / **`sys_enter_execveat`**; rules use **`/proc/<tgid>/cmdline`** haystack). **`MACE_EBPF_EXECVE_FULL_ARGV=1`** enables ring **v14** in-kernel capture (up to **15** args × **127** bytes per arg, **800**-byte argv blob after header; shared wire for **`execve`** and **`execveat`**). **`is_truncated`** indicates overflow. **`BPF_PROG_LOAD` EACCES** may be lockdown/capabilities. **`MACE_EBPF_EXECVE_ARGV0_ONLY=1`** is an intermediate shrink if full argv fails verifier load.
