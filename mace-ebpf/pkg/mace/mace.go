// Package mace provides Go bindings for the Mace-eBPF SDK.
//
...
// Static link (no libmace_ebpf.so). Default: debug artifact — `cargo build -p mace-ebpf`.
...
var ErrClosed = errors.New("mace: sensor closed")
...
// errFromArenaCode converts a C result from arena try_push / try_pop (MaceErrorCode).
...
// recvAlertResult interprets mace_alert_channel_try_recv return values:
