//! Measures extra wall-clock time for `mprotect` RWX→RW cycles with Mace tracepoints attached
...
//!   cargo test --release --test ebpf_overhead_bench -p mace-ebpf -- --ignored --nocapture
//! ```
//!
...
//!
//! ```text
//! MACE_MPROTECT_OVERHEAD_NS_MAX=1000 sudo ... cargo test --release ...
//! ```
...
fn test_mprotect_latency_overhead_with_ebpf() {
    ...
    let slowdown_max: f64 = std::env::var("MACE_MPROTECT_SLOWDOWN_MAX")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(DEFAULT_SLOWDOWN_MAX);
    assert!(
        slowdown_max > 1.0,
        "MACE_MPROTECT_SLOWDOWN_MAX must be > 1.0, got {slowdown_max}"
    );

    let max_overhead_ns_opt: Option<u64> = std::env::var("MACE_MPROTECT_OVERHEAD_NS_MAX")
        .ok()
        .and_then(|s| s.parse().ok());
    ...
    assert!(
        ...
        "eBPF mprotect slowdown too high: {slowdown:.4}x > {slowdown_max}x; \
         raise MACE_MPROTECT_SLOWDOWN_MAX if needed, or use --release for tighter numbers"
    );
