...
//!
//! `sudo env PATH="$PATH" RUSTUP_HOME="$RUSTUP_HOME" CARGO_HOME="$CARGO_HOME" cargo test --test tracepoint_attach_test -p mace-ebpf -- --ignored`
//!
//! The test sleeps briefly after `mmap` so `mprotect` is not dropped by the eBPF program’s shared
//! mmap/mprotect rate limiter (see `RATE_LIMIT_INTERVAL_NS` in `mace-ebpf-ebpf`).
...
/// `(program_name, tracepoint_name)` — same pairing as `start_sensor` in `mace-ebpf` lib.
