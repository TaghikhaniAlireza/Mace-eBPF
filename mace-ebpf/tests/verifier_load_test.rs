...
//! such as `mace_ebpf`; those are skipped because they are not valid `BPF_PROG_TYPE_TRACEPOINT`
//! loads.)
//!
...
//!
//! `sudo env PATH="$PATH" RUSTUP_HOME="$RUSTUP_HOME" CARGO_HOME="$CARGO_HOME" cargo test --test verifier_load_test -p mace-ebpf -- --ignored`
...
fn test_load_ebpf_program() {
    ...
    assert!(
        ...
        "no tracepoint programs found in {}; check mace-ebpf-ebpf exports #[tracepoint] progs",
        path.display()
    );
